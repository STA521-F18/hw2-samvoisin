...
ggplot(data = PreTMod, aes(x = .resid)) +
  ...
  labs(x = "Residuals",
       y = "Residual Density",
       subtitle = "Residuals skew right as depicted by Normal Q-Q plot") +
...
Notably, the AV plot for `Purban` has a slope that is almost zero meaning that it does not provide unique information regarding the variation in `ModernC` beyond the other predictors. It should be removed from the model for parsimony.
...
The first transformation I performed is taking the natural log of the `Pop` variable. This brings the data within a single order of magnitude (Fig. 6.1). 
