...
There were 125 observations used in fitting this preliminary model due to the necessary removal of incomplete cases.
...
Notably, the added variable plots for `Purban` and `Frate` have slopes near zero meaning that they do not provide unique information regarding the variation in `ModernC` beyond the other predictors. I chose to remove these predictors from the model for parsimony.
...
The added variable plots show that all four predictors provide unique information regarding the sources of variability for the response.

Diagnostic plots of the predictor/ response relationsips are below. Most notably, the Normal Q-Q plot shows a decrease in the leftward skew of residuals and the leverage values of China and India have decreased as intended on the Residuals vs Leverage plot. 
...
ggplot(PostXTMod, aes(x = hatvalues(PostXTMod), y = rstandard(PostXTMod))) + 
  ...
  geom_text(aes(label = ifelse(hatvalues(PostXTMod) > 1,
...
Cook_Dist <- cooks.distance(PostXTMod)
