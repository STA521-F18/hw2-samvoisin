...
As a first step, I used the ggpairs to create a matrix of all of the variables. From the plot matrix, It is clear that there are two outliers in `Pop`. These points represent China and India. The next observation I made from the plot matrix is the relationship `Fertility` has with several of the variables. It seems that `Fertility` has a slightly exponential relationship with both `ModernC` and `Change`. This relationship becomes more extreme for `PPgdp`. Finally, `Purban` has an almost linear, positive relationship with `ModernC`
...
boxTidwell(y = UN3_rmna$ModernC, x1 = UN3_rmna$Purban)
