...
ModcFemR <- ggplot(data = UN3_rmna, aes(x = Frate, y = ModernC)) + 
  geom_point(size = 2.5) +
  geom_smooth(color = "red", se = FALSE) +
  ggtitle("Females in Economy vs Modern Contraception %") +
  labs(x = "Economically Active females (%)",
       y = "Access to Contraception (%)") +
  theme(plot.title = element_text(size = 18, face = "bold"),
        axis.title = element_text(size = 17),
        axis.text = element_text(size=18))

grid.arrange(PopModc, PPgdpModc, ModcFert, ModcFemR, ncol = 2)
...
Notably, the AV plot for `Purban` has a slope that is almost zero meaning that the variable does not provide additional unique information to the model beyond the other variables. I will continue to monitor this variable as I do transformations to determine whether I should remove `Purban` from the model.
...
The first transformation I perform is taking the natural log of the `Pop` variable. This brings the data within a single order of magnitude (Fig. 6.1). 

```{r Question6, echo = FALSE, fig.width= 14, fig.height= 10}

UN3_rmna %>%
  # boxTidwell function cannot take negative values
  # shift Change var up by min value + 1
  mutate(ShiftChange = Change + min(Change) + 1) %>%
  boxTidwell(ModernC ~ PPgdp + Fertility + ShiftChange,
             other.x = ~ log(Pop) + Frate,
             data = UN3_rmna)
...
SChangeModc <- ggplot(data = UN3_rmna,
  ...
  labs(x = "Shifted Population Change",
       y = "Access to Contraception (%)") +
  theme(plot.title = element_text(size = 18, face = "bold"),
        axis.title = element_text(size = 17),
          axis.text = element_text(size=18))

grid.arrange(PopModc, ModcFemR, lPPgdpModc, SChangeModc, ncol = 2)

avPlots(PostXTMod)

par(mfrow=c(2,2))
plot(PostXMod, ask = FALSE)
